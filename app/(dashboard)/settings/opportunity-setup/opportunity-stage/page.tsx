...
export default function OpportunityStagesPage() {
  ...
  const tableColumns: TableColumn<OpportunityStage & { sNo: number }>[] = columns.map((col) => ({
    key: col.key as keyof (OpportunityStage & { sNo: number }),
    label: col.label,
    visible: col.visible,
    render: (row: OpportunityStage & { sNo: number }) => {
      if (col.key === "sNo") {
  ...
  const paginatedOpportunityStages = filteredOpportunityStages
